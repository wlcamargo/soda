...
from functions import read_and_concat_csv
...
app_name = 'ingestion-csv-bigquery'
...
def execute():
    ...
    add_metadata(df_source, source="csv", tool="python")

    #Call the function that generates the schema of the table to be created.
    #generate_bigquery_schema(df_source)

    table_schema = [
    bigquery.SchemaField('IDTransacao', 'INTEGER'),
        bigquery.SchemaField('Cliente', 'STRING'),
        bigquery.SchemaField('NumeroDoCartao', 'STRING'),
        bigquery.SchemaField('DataDaTransacao', 'STRING'),
        bigquery.SchemaField('ValorDaTransacao', 'FLOAT'),
   ...
   # Authenticate for BigQuery
    ...
    table_id_bigquery = 'transactions'
